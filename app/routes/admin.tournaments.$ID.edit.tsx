...
export async function action({ request }: ActionFunctionArgs) {
    ...
    if (data) {
        const { headers } = await setToast({ request, toast: 'success::The tournament has been updated' })
        return redirect('/admin/tournaments', { headers })
    } else if (error) {
        const { headers } = await setToast({ request, toast: `error::${error.detail}` })
        return json(error, { headers })
    }
    const { headers } = await setToast({ request, toast: `error::Sorry, this tournament no longer exists` })
    return redirect('/admin/tournaments', { headers })
}
